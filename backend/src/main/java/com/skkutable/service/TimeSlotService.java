...
public class TimeSlotService {
  ...
  public TimeSlotResponseDto createTimeSlot(Long boothId, TimeSlotCreateDto dto, String userEmail) {
    ...
    // 중복 시간대 검증
    if (timeSlotRepository.existsByBoothIdAndStartTimeAndEndTime(boothId, dto.getStartTime(),
        dto.getEndTime())) {
      throw new ConflictException("이미 동일한 시간대의 타임슬롯이 존재합니다");
    }

    TimeSlot timeSlot = TimeSlot.builder().booth(booth).startTime(dto.getStartTime())
        .endTime(dto.getEndTime()).maxCapacity(dto.getMaxCapacity()).currentCapacity(0)
  ...
  public TimeSlotResponseDto updateTimeSlot(Long boothId, Long timeSlotId, TimeSlotUpdateDto dto,
      String userEmail) {
    User user = userService.getCurrentUser(userEmail);
    Booth booth = boothService.findBoothById(boothId);
    ...
    // 예약이 있는 경우 시간 변경 불가
    if (timeSlot.getCurrentCapacity() > 0 && (dto.getStartTime() != null
        || dto.getEndTime() != null)) {
    ...
    if (dto.getStartTime() != null && dto.getEndTime() != null) {
      ...
      validateTimeSlot(dto.getStartTime(), timeSlot.getEndTime(), booth);
    } else if (dto.getEndTime() != null) {
      validateTimeSlot(timeSlot.getStartTime(), dto.getEndTime(), booth);
    }
    // 업데이트 처리
  ...
  public List<TimeSlotResponseDto> getTimeSlotsByBooth(Long boothId) {
    ...
    return timeSlots.stream().map(TimeSlotResponseDto::from).collect(Collectors.toList());
  }
  ...
  public TimeSlot findTimeSlotByIdAndBoothId(Long id, Long boothId) {
    return timeSlotRepository.findByIdAndBoothId(id, boothId).orElseThrow(
