...
public class ReservationServiceV2 {
  ...
  public ReservationResponseDTO createReservation(ReservationRequestDTO dto, String userEmail) {
    ...
    TimeSlot timeSlot = timeSlotRepository.findById(dto.getTimeSlotId())
        .orElseThrow(() -> new ResourceNotFoundException("TimeSlot not found: " + dto.getTimeSlotId()));

    // 예약 가능 시간 확인
    if (booth.getReservationOpenTime() != null && 
        LocalDateTime.now().isBefore(booth.getReservationOpenTime())) {
    ...
    if (!timeSlot.canAccommodate(dto.getNumberOfPeople())) {
      throw new BadRequestException("남은 자리가 부족합니다. 현재 가능 인원: " + 
          (timeSlot.getMaxCapacity() - timeSlot.getCurrentCapacity()));
    }

    // 중복 예약 검증
    boolean alreadyReserved = reservationRepository.existsByUserIdAndTimeSlotId(
        user.getId(), timeSlot.getId());
    if (alreadyReserved) {
  ...
  public List<ReservationResponseDTO> getTimeSlotReservations(Long timeSlotId, String userEmail) {
    ...
    if (user.getRole() == com.skkutable.domain.Role.HOST && 
        !booth.getCreatedBy().getId().equals(user.getId())) {
